...
class Curve25519Test extends BaseTestCase {
    ...
    public function test_zero_private_key() {
        ...
        assert("Public key should be generated", publicKey != null);
    }
    ...
    public function test_invalid_key_lengths() {
        try {
            ...
            assert("Should have thrown exception for short private key", false);
        } catch (e:Dynamic) {
            assert("Should throw for invalid private key length", true);
        }
        ...
        try {
            ...
            assert("Should have thrown exception for short public key", false);
        } catch (e:Dynamic) {
            assert("Should throw for invalid public key length", true);
        }
    }
    ...
    public function test_different_inputs() {
        ...
        assert("Public keys should be different", !bytesEqual(pub1, pub2));
    }
    ...
    private function assertBytesEqual(message:String, a:Bytes, b:Bytes):Void {
        if (a.length != b.length) {
            assert(message + " - Lengths differ", false);
            return;
        }
        
        for (i in 0...a.length) {
            if (a.get(i) != b.get(i)) {
                assert(message + " - Bytes at index " + i + " differ", false);
                return;
            }
        }
        assert(message, true);
